...
pub fn workspace_create(
    ...
    conn.execute(
        ...
        params![id, args.name, args.migration_repo_path, created_at],
    )
    .map_err(|e| {
...
pub fn workspace_get(state: State<DbState>) -> Result<Option<Workspace>, CommandError> {
    ...
    let result = conn.query_row(
        "SELECT id, display_name, migration_repo_path, created_at FROM workspaces LIMIT 1",
        [],
        |row| {
            Ok(Workspace {
                id: row.get(0)?,
...
mod tests {
    ...
    fn create_and_get_workspace() {
        ...
        conn.execute(
            "INSERT INTO workspaces(id, display_name, migration_repo_path, created_at) VALUES (?1, ?2, ?3, ?4)",
            rusqlite::params![id, "Test", "/tmp/repo", "2026-01-01T00:00:00Z"],
        )
        .unwrap();
        let w: Workspace = conn
            .query_row(
                "SELECT id, display_name, migration_repo_path, created_at FROM workspaces WHERE id=?1",
                rusqlite::params![id],
                |row| {
                    Ok(Workspace {
                        id: row.get(0)?,
