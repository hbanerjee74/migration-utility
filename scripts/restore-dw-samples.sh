...
BACKUP_DIR="${BACKUP_DIR:-$HOME/sql-backups}"
CONTAINER_BACKUP_DIR="/var/opt/mssql/backup"
SQLCMD_ARGS=(-S "$SQL_SERVER_HOST" -U sa -P "$SA_PASSWORD" -C -W -h -1 -s "|")
...
ensure_backup_present() {
  ...
  if [[ ! -f "$local_path" ]]; then
    ...
    echo "Using existing backup: $local_path"
  fi
...
restore_database_from_backup() {
  ...
  while IFS='|' read -r logical_name physical_name _rest; do
    ...
    # FILELISTONLY can include Windows paths and multiple filegroups.
    # Normalize to Linux container path and preserve original filename.
    local target_file
